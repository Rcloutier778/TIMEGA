    ...
    public void damageVal () {
        for(int i = 0; i < 2; i++) {
            ...
            if(Database.hasTechLocal(_names[i], "Cybernetics")) {
                _unitHitRate[i][Database.FIGHTER] -= 1;
            }
            ...
            if(Database.raceOf(_names[i]).equals("The Sardakk N'Orr")){
                for(int k=0; i<Database.NUM_SHIPS; k++) {
                    _unitHitRate[i][k] -= 1;
                }
            }
            
            if(Database.raceOf(_names[i]).equals("The L1Z1X Mindnet")){
                _unitHitRate[i][Database.DREADNOUGHT] -= 1;
            }
        }
    }
            ...
            for (int k = 0; k < Database.NUM_SHIPS; k++) {
                while (preFire[i] > 0) {
                    if (_unitCounts[e][k] > 0) {
                        _unitCounts[e][k] -= 1;
                        preFire[i] -= 1;
                        if (k == Database.DREADNOUGHT) {
                            if (Database.hasTechLocal(_names[i], "Transfabrication")) {
                                _unitCounts[e][Database.DESTROYER] += 1;
                                k -= 2;
                            }
                        }
                    } else {
            ...
            for (int k = 0; k < Database.NUM_SHIPS; k++) {
                while (cruiserHits[i] > 0) {
                    if (_unitCounts[e][k] > 0) {
                        _unitCounts[e][k]--;
                        cruiserHits[i]--;
                        if (k == Database.DREADNOUGHT && Database.hasTechLocal(_names[i], "Transfabrication")) {
